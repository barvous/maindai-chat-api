...
const FirestoreService = {
  ...
  async listarMensagensDaSala(salaId) {
    const mensagensRef = db.collection('sala').doc(salaId).collection('mensagens');
  ...
  const snapshot = await db.collection('sala')
  ...
  async adicionarMensagemNaSala(salaId, { usuario, texto, data_envio }) {
    const novaMensagem = {
      usuario,
      texto,
      data_envio: data_envio || new Date().toISOString()
    };
    const ref = await db.collection('sala').doc(salaId).collection('mensagens').add(novaMensagem);
  ...
  // (Opcional) Pega o assunto da sala
  async obterAssuntoSala(salaId) {
    const doc = await db.collection('sala').doc(salaId).get();
    if (!doc.exists) return null;
    return doc.data().assunto || null;
  }
};
